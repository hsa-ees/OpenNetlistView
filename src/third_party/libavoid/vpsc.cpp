...
bool IncSolver::satisfy()
{
          ...
          (v->equality || ((v->slack() < ZERO_UPPERBOUND) && !v->active)))
    {
        ...
        else
        {
            ...
            try
            {
                Constraint* splitConstraint = lb->splitBetween(v->left, v->right, lb, rb);
...
Blocks::Blocks(vector<Variable*> const& vs)
    : vs(vs)
    , nvs(vs.size())
{
    blockTimeCtr = 0;
 ...
 */
bool Block::getActivePathBetween(Constraints& path, Variable const* u, Variable const* v, Variable const* w) const
{
    if(u == v)
        return true;
...
std::ostream& operator<<(std::ostream& os, const Constraint& c)
{
    ...
    {
        os << "(" << c.slack() << ")" << (c.active ? "-active" : "") << "(lm=" << c.lm << ")";
    }
    else
    {
...
bool CompareConstraints::operator()(
    Constraint* const& l, Constraint* const& r) const
{
    double const sl =
