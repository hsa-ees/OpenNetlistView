...
void dijkstra(const unsigned s, const unsigned n, double* d, const vector<Edge>& es, const std::valarray<double>& eLengths)
{
...
void ConstrainedFDLayout::computeDescentVectorOnBothAxes(
    const bool xAxis, const bool yAxis, double stress, Position& x0, Position& x1)
{
    setPosition(x0);
    if(xAxis)
    {
...
void ConstrainedFDLayout::recGenerateClusterVariablesAndConstraints(
    vpsc::Variables (&vars)[2], unsigned int& priority, cola::NonOverlapConstraints* noc, Cluster* cluster, cola::CompoundConstraints& idleConstraints)
{
        ...
        ++curr)
    {
        ...
        recGenerateClusterVariablesAndConstraints(vars, priority, noc, *curr, idleConstraints);
    }
    ...
    if((noc == nullptr) && (dynamic_cast<RootCluster*>(cluster) == nullptr))
    {
        ...
        cola::ClusterContainmentConstraints* ccc =
            new cola::ClusterContainmentConstraints(cluster, priority, boundingBoxes);
    ...
    if(noc)
    {
            ...
            ++curr)
        {
            ...
            // constraints.
            noc->addShape(id, boundingBoxes[id]->width() / 2, boundingBoxes[id]->height() / 2, group);
        }
        ...
        {
            unsigned id = *curr;
            noc->addShape(id, boundingBoxes[id]->width() / 2, boundingBoxes[id]->height() / 2, group + 1);
        }
    }
}
    ...
    vpsc::Variables (&vs)[2])
{
    if(clusterHierarchy && !clusterHierarchy->flat())
    {
        ...
        recGenerateClusterVariablesAndConstraints(vs, priority, nullptr, clusterHierarchy, extraConstraints);
        ...
        if(m_generateNonOverlapConstraints)
        {
            ...
            recGenerateClusterVariablesAndConstraints(vs, priority, noc, clusterHierarchy, extraConstraints);
    ...
    else if(m_generateNonOverlapConstraints)
    {
        ...
        {
            noc->addShape(i, boundingBoxes[i]->width() / 2, boundingBoxes[i]->height() / 2);
        }
        extraConstraints.push_back(noc);
    }
}
...
void ConstrainedFDLayout::makeFeasible(double xBorder, double yBorder)
{
    ...
    for(unsigned int dim = 0; dim < 2; ++dim)
    {
        ...
        {
            double pos = (dim == 0) ? boundingBoxes[i]->getCentreX() : boundingBoxes[i]->getCentreY();
    ...
    std::sort(idleConstraints.begin(), idleConstraints.end(), cmpCompoundConstraintPriority);
...
void setupVarsAndConstraints(unsigned n, const CompoundConstraints& ccs, const vpsc::Dim dim, vpsc::Rectangles& boundingBoxes, RootCluster* clusterHierarchy, vpsc::Variables& vs, vpsc::Constraints& cs, valarray<double>& coords)
{
    vs.resize(n);
    for(unsigned i = 0; i < n; ++i)
    {
...
void setVariableDesiredPositions(vpsc::Variables& vs, vpsc::Constraints& cs, const DesiredPositionsInDim& des, valarray<double>& coords)
{
    COLA_UNUSED(cs);
...
{
    topologyAddon->handleResizes(resizeList, n, X, Y, ccs, boundingBoxes, clusterHierarchy);
}
...
void ConstrainedFDLayout::moveTo(const vpsc::Dim dim, Position& target)
{
    ...
    vpsc::Variables vs;
    vpsc::Constraints cs;
    setupVarsAndConstraints(n, ccs, dim, boundingBoxes, clusterHierarchy, vs, cs, coords);
...
double ConstrainedFDLayout::applyForcesAndConstraints(const vpsc::Dim dim, const double oldStress)
{
    ...
    if(topologyAddon->useTopologySolver())
    {
        stress = topologyAddon->applyForcesAndConstraints(this, dim, g, vs, cs, coords, des, oldStress);
    }
    else
    {
        // Add non-overlap constraints, but not variables again.
...
void ConstrainedFDLayout::outputInstanceToSVG(std::string instanceName)
{
    ...
    for(size_t i = 0; i < boundingBoxes.size(); ++i)
    {
        fprintf(fp, "    rect = new vpsc::Rectangle(%g, %g, %g, %g);\n", boundingBoxes[i]->getMinX(), boundingBoxes[i]->getMaxX(), boundingBoxes[i]->getMinY(), boundingBoxes[i]->getMaxY());
...
ProjectionResult projectOntoCCs(Dim dim, Rectangles& rs, CompoundConstraints ccs, bool preventOverlaps, int accept, unsigned debugLevel)
{
    size_t n = rs.size();
        ...
        for(Constraints::iterator it = cs.begin(); it != cs.end(); it++)
        {
            ...
            if(c->unsatisfiable)
            {
                ...
                if((unsigned)c->left->id < rs.size())
                {
                    ...
                    sprintf(buf, "    v_%d rect: [%f, %f] x [%f, %f]\n", c->left->id, r->getMinX(), r->getMaxX(), r->getMinY(), r->getMaxY());
                    unsatinfo += buf;
                }
                if((unsigned)c->right->id < rs.size())
                {
                    Rectangle* r = rs[c->right->id];
                    sprintf(buf, "    v_%d rect: [%f, %f] x [%f, %f]\n", c->right->id, r->getMinX(), r->getMaxX(), r->getMinY(), r->getMaxY());
                    unsatinfo += buf;
                }
                CompoundConstraint* cc = (CompoundConstraint*)(c->creator);
